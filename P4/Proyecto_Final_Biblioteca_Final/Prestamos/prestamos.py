...
def mostrarPrestamosAlumno(matricula, conexionBD): 
    print("\033c")
    texto = f"{funciones.AZUL}" + "....:::: LISTA DE MIS PRÉSTAMOS ::::....".center(80) + f"\n{funciones.RESET}"
    funciones.escritura_lenta_print(texto)
    
    prestamos = Prestamos.crud.consultarAlumno(matricula, conexionBD)
    
    if len(prestamos) > 0:
        print(f"  {funciones.AMARILLO}{'ID':<5}{'Usuario':<10}{'Libro':<18}{'F. Préstamo':<14}{'F. Devolución':<15}{'Estado':<10}{'Observaciones':<20}{funciones.RESET}\n")
        ...
        for p in prestamos:
            ...
            f_dev = str(p[4])[:12]
            estado = str(p[5])[:9]
            obs = str(p[6])[:20] if p[6] else "Sin observaciones"
            
            print(f"  {id_p:<5}{usr:<10}{libro:<18}{f_prest:<14}{f_dev:<15}{estado:<10}{obs:<20}")
